...
export default function Dashboard() {
  const { currentFront, getAlter, alters, tasks, messages, journalEntries, calendarEvents, safetyPlans } = useSystem();
          ...
          <CardHeader className="pb-3">
            <CardTitle className="text-lg font-heading flex items-center gap-2">
              ...
              {unreadMessages.length > 0 && (
                <Badge className="bg-primary text-primary-foreground">{unreadMessages.length} new</Badge>
              )}
            </CardTitle>
          </CardHeader>
          <CardContent>
                ...
                return (
                  <li key={msg.id} className="text-sm space-y-0.5">
                    <div className="flex items-center gap-1.5">
                      {from && (
                        <span className="font-medium" style={{ color: from.color }}>{from.emoji} {from.name}</span>
                      )}
