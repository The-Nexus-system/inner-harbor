      ...
      {safetyPlans.map(plan => (
        <Card key={plan.id} aria-label={plan.title}>
          ...
          <CardContent className="space-y-4">
                ...
                <ul className="space-y-1.5 text-sm">
                  {plan.trustedContacts.map((contact, i) => (
                    <li key={i} className="flex items-center gap-2">
                      ...
                      {contact.phone && (
                        <a href={`tel:${contact.phone}`} className="text-primary underline tap-target">{contact.phone}</a>
                      )}
                    </li>
                  ))}
                </ul>
            ...
            {plan.notes && (
              <p className="text-sm text-muted-foreground italic border-t border-border pt-3">{plan.notes}</p>
            )}
          </CardContent>
        </Card>
      ))}
