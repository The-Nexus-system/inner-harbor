...
import type { FrontStatus, MemoryContinuity } from "@/types/system";
...
export default function FrontPage() {
  const { alters, frontEvents, currentFront, getAlter, setCurrentFronter } = useSystem();
    ...
    <div className="space-y-6 max-w-5xl mx-auto animate-fade-in">
      ...
      <Card aria-label="Quick switch tool">
        ...
        <CardContent className="space-y-4">
          ...
          <fieldset>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
              {alters.filter(a => a.isActive).map(alter => (
                <label
                  key={alter.id}
                  className="flex items-center gap-3 p-3 rounded-lg border border-border hover:bg-muted cursor-pointer tap-target"
                >
                  <Checkbox
                    checked={selectedAlters.includes(alter.id)}
                    onCheckedChange={() => toggleAlter(alter.id)}
                    aria-label={`Select ${alter.name}`}
                  />
                  <span
          ...
          <Button onClick={handleSwitch} className="tap-target" size="lg">
            Update front
          </Button>
        </CardContent>
      </Card>
